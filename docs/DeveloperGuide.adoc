...
.. `DateTimeParser` finds a list of available time periods within the date range identified in low-level parsing, and passes the list as a `String` into `Prompt` and `UI` objects. These then display the results to the user.
...
* `Diagnosis#addMatcher(Disease disease, Set<Symptom> symptoms)` – Store a particular disease-symptom matcher input by users.
* `Diagnosis#predictDisease(Set<Symptom> symptoms)` – Predict a disease for symptoms input by users.

These operations are exposed in the DiagnosisModel interface as `DiagnosisModel# hasDisease(Disease disease)`,
`DiagnosisModel#getSymptoms(Disease disease)`, `DiagnosisModel#addMatcher(Disease disease, Set<Symptom> symptoms)`, and
...
`HashMap<Disease,Set<Symptom>> matcher` stored in the database.
...
Inside the method, `Diagnosis#addMatcher(Disease disease, Set<Symptom> symptoms)` will be
invoked and reset the private attribute `matcher`.

Step 4. Now the user decides to execute the command `predict` to search for a disease in database
that matches with input symptoms. This command calls `DiasnosisModel#predictDisease(Set<Symptom> symptoms)`
