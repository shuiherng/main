...
Step 3. Time Parsing:  User input for time is parsed into a `Pair<Calendar, Calendar>` object.
...
. Breaking Down: The user input string, starting with `for`, is passed into a `ScheduleEventParser` object, and broken down into meaningful substrings for patient and time respectively:
...
. Time Parsing: The string `next week` is passed to a `DateTimeParser` object where it is parsed and converted into a `Pair<Calendar, Calendar>` object to represent the user's chosen time slot for the appointment:
...
. Further Prompting: The user is presented with two more `PromptWindow`, where they can provide further inputs for tagsand additional notes for the appointment. This is done through a simple I/O mechanism.
...
The sequence diagram below illustrates this process, showing notable steps. Note that step 4 and 5 are omitted in the diagram as they are relatively trivial:
