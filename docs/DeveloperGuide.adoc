...
This feature facilitates scheduling, which enables the user to add appointments in the schedule system with user input phrased in natural language, and does so in a conversational process. It mainly implements a command parser which parses natural language user input, and creates the intended `ScheduleEvent` object to be stored.

There are four steps involved in the processing of natural language commands:

. High-Level Parsing: Command-specific, user inputs are broken down into sub-fields
. Low-Level Parsing: Date/time specific, user inputs for date or duration are parsed into a `Pair<Calendar, Calendar>` object
. Further Prompting: User is prompted with a list of available time slots, and required to select a specific time slot
. Generating Event: The resulting `ScheduleEvent` object corresponding to the user input is created and sent to `Schedule` to be stored.

The following is an example of a use case, and how the mechanism behaves:

User Input: `schedule for David Lee next week`.

[NOTE]
Unlike other commands, the `schedule` command does not require prefixing each subsequent field. This is to simulate natural language, and eliminates the need for a prefix reference sheet when scheduling appointments for patients.

. High-Level Parsing: The entire string is passed into a `ScheduleCommandParser` object, and broken down into meaningful sub-strings:
.. `schedule` is the command word, and is discarded.
.. `David Lee` is the name of the patient. The corresponding patient's `PersonID` will be obtained by searching through the list of patients stored in `PersonList`. Should there be multiple patients with the same name, the application will prompt the user to select the appropriate one. Once the `PersonID` is obtained, it is stored as an attribute in a new `ScheduleEvent` object.
.. `next week` is the date/duration keyword, and is carried forward to the next step for further parsing.

. Low-Level Parsing: The date/duration string is parsed and converted into a `Pair<Calendar, Calendar>` object to represent `next week`:
.. The program executes a keyword search and invokes the method `getWeekDates(currentTime, 1)`, where it converts `next week` into a date range, by doing relevant calculations on a `Calendar` object which represents the current time. For instance, if the command is executed on 16/10/2018, `next week` becomes a datetime range from 22/10/2018 0859hrs to 28/10/2018 1801hrs. This range takes into consideration the doctor's working hours.
.. Two `Calendar` objects are created in the process to represent the start and end datetimes. These two objects are then placed in a `Pair`.

. Further Prompting: The user is presented with a list of suggested empty time slots for the appointment, from which one is chosen:
.. `DateTimeParser` finds a list of available time periods within the datetime range identified in low-level parsing, and passes the list as a `String` into `Prompt` and `UI` objects. These then display the results to the user.

.. The user then inputs the desired time slot such as `22/10/2018 9:30-10:30`.
.. The refined time selection is then passed back to `DateTimeParser` and converted into a `Pair<Calendar, Calendar>` object.

. Generating Event: The `Pair` object from the previous step is added as an attribute in the `ScheduleEvent` object which is then returned.

[NOTE]
Instead of using natural language, the user may also choose to enter a `schedule` command in a fixed format such as `schedule for David Lee *on* 22/10/2018 0930 *to* 1030` to create an appointment without further prompts.

The sequence diagram below illustrates this process, showing notable steps:
...
===== Aspect (Coming in v1.4): How undo & redo executes
...
===== Aspect (Coming in v1.4): Data structure to support the undo/redo commands
