...
Step 1. Breaking Down: User input is broken down into sub-fields, namely, name and date/duration.

Step 2. Patient Parsing:  User input for patient name is parsed into the corresponding `PersonId` object.

Step 3. Date/duration Parsing:  User input for date/duration is parsed into a `Pair<Calendar, Calendar>` object.
...
. Breaking Down: The user input string, starting with `for`, is passed into a `ScheduleEventParser` object, and broken down into meaningful substrings for name and date/duration respectively:

.. Programme starts with assuming that the substring for name is only one word long, and the remaining string following the name word all the way to the end of the string is the input for date/duration. In this case, `David` is the assumed name substring and `Lee next week` is the assumed date/duration substring.
.. It takes the assumed date/duration substring,`Lee next week`, and checks if it is a valid date/duration expression.
.. As a match cannot be found, it means that the assumed demarcation between name and date/duration inputs is incorrect. Programme makes another attempt by assuming the name substring is longer by one word (i.e. `David Lee`) and the date/duration substring is shorter by one word (i.e. `next week`). It takes the new assumed date/duration substring and checks its validity again.
.. As a match is found this time, it indicates that the assumption is correct. `David Lee` will be carried forward to the Patient Parsing step while `next week` will be carried forward to the Date/duration Parsing step.
...
. Patient Parsing: The string `David Lee` is parsed and converted into the corresponding `PersonId` object:
.. TO-DO

. Date/duration Parsing: The string `next week` is passed to a `DateTimeParser` object where it is parsed and converted into a `Pair<Calendar, Calendar>` object to represent the user's chosen time slot for the appointment:
...
.. Programme searches the list of already scheduled appointments within the datetime range obtained. It finds a list of available time periods by taking the complement within that range, and passes the list as a `String` into a `Prompt` object, where the list is displayed to the user in a `PromptWindow`.
.. User inputs a specific time slot from the list of available time periods. For instance, user inputs `22/10/2018 09:00 - 10:00`. The refined time slot string is then passed back to `DateTimeParser` and converted into a `Pair<Calendar, Calendar>` object that represents this time slot, by invoking the method `parseTimeSlot(timeSlotString)`.

. Further Prompting: The user is presented with two more `PromptWindow`, where they can provide further inputs for tag and additional notes for the appointment. This is done through a simple I/O mechanism.

. Generating Appointment: Results from the previous steps are used to fill the attributes of a newly created `ScheduleEvent` object which is then returned.


The sequence diagram below illustrates this process, showing notable steps: (DIAGRAM TO BE CHANGED)
