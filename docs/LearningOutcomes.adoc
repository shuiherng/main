...
** `Model` component contains the 'Model'. However, note that it is possible to view the `Logic` as the addressBookModel because it hides the `Model` behind it and the view has to go through the `Logic` to access the `Model`.
